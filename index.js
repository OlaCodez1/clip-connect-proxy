// Node.js (Express) Proxy Server - Updated with AES key support and session cleanup
...
app.post('/create-session', async (req, res) => {
  ...
  while (tries < 5) {
    ...
    if (!error) return res.json({ sessionId: data.id, code, aesKey: key });
...
app.post('/sync', async (req, res) => {
  ...
  const { error } = await supabase.from('clipboard_items').insert([
    { session_id: sessionId, content: clip }
  ]);
...
app.get('/latest/:sessionId', async (req, res) => {
  ...
  if (error || !data) return res.status(404).json({ error: 'No clipboard found' });
...
app.post('/end-session', async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: 'Failed to end session' });
  res.json({ success: true });
});
